...
const WorkoutPaginate = ({ pages, page, isAdmin = false }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [windowWidth, setWindowWidth] = useState(window.innerWidth);

  // Get current query parameters to preserve them during pagination
  ...
  const handleClick = (pageNumber) => {
    // Navigate to the new page while preserving query parameters
    const searchParams = queryParams.toString();
    const baseUrl = isAdmin ? '/admin/workouts' : '/workouts';
    const pageUrl = pageNumber === 1 ? baseUrl : `${baseUrl}/page/${pageNumber}`;
    navigate(searchParams ? `${pageUrl}?${searchParams}` : pageUrl);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="ms-1 d-none d-md-inline">Prev</span>
      </Pagination.Prev>
...
export default WorkoutPaginate;