...
const UserPaginate = ({ pages, page, keyword = '', role = '', codeAccess = '' }) => {
  ...
  const handleClick = (pageNumber) => {
    ...
    if (codeAccess && codeAccess !== 'all') params.set('codeAccess', codeAccess);

    // Navigate to the new page with filters
    const queryString = params.toString() ? `?${params.toString()}` : '';
    const pageUrl = pageNumber === 1 ? '/admin/userlist' : `/admin/userlist/${pageNumber}`;
